...
rot!(x) = x[1], x[2] = x[2], -x[1]
refl!(x) = x[1], x[2] = x[2], x[1]
rot2!(x) = x[1], x[2] = -x[1], -x[2]
rot3!(x) = x[1], x[2] = -x[2], x[1]
...
function Lorentz3D2(x, v, r)
    ...
    while (xr == false)
        ...
        #        while (int(tmax)>int(tmin+BigFloat("0.5")) )   # can be done by considering coincidence in time

        while x1[1]!=x2[1] || x1[2]!=x3[1] || x2[2]!=x3[2]    # or coincidence of obstacle

            # positions in planes xy, xz, and yz:
            x_xy = [x[1], x[2]]
            x_xz = [x[1], x[3]]
            x_yz = [x[2], x[3]]

            x1 = Lorentz2(x_xy, vn_xy, r) # calculate the obstacle that the particle collides with
            if x1 == false
                return false
            end
